...
export class FlipWizardComponent extends BaseWizardComponent implements AfterViewInit {
	...
	animateDateForm(show: boolean, element: HTMLElement) {
		if (show) {
			// Show animation with flip effect
			gsap.to(element, {
				display: 'block',
				rotateX: 0,
				opacity: 1,
			...
			// Hide animation with flip effect
			gsap.to(element, {
				rotateX: -90, // Changed to -90 to flip out from the top
				opacity: 0,
				duration: 0.7,
				ease: 'power2.in',
				transformOrigin: 'top center', // Changed to top center
				onComplete: () => {
